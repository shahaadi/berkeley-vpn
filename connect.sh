...
usage() {
    ...
    cat <<EOF
Berkeley VPN: connect via openconnect + GlobalProtect, without the GlobalProtect app.
...
Tunnels:
  ...
  restricted   Optional add-on for high-security P4 access. First use asks to
               install a small helper and needs Information Security Office
               (ISO) approval; most people don't need this.
               See '$CMD restricted'.
...
Environment variables:
  ...
  GP_TIMEOUT=<seconds>   how long to wait for the login window (default 240)
...
do_update() {
    ...
    if [ -d "$HERE/.git" ]; then
        ...
        local oldver; oldver="$(read_version)"
        git -C "$HERE" pull --ff-only --quiet || { echo "!! git pull failed." >&2; exit 1; }
        echo "$CMD updated (version $oldver -> $(read_version))."
        exit 0
    fi
    command -v curl >/dev/null 2>&1 || { echo "!! curl not found." >&2; exit 1; }
    ...
    # A newer version is available, so download the scripts (and the new VERSION).
    ...
    if [ "$ok" = 1 ]; then
        ...
        if mv -f "$tmp/capture.swift" "$HERE/capture.swift" \
           ...
           && mv -f "$tmp/VERSION" "$HERE/VERSION"; then
            ...
            rm -rf "$tmp"
            echo "$CMD updated (version $local_ver -> $remote_ver)."
            exit 0
        fi
...
case "$ACTION" in
    split)      GW="campus-split.vpn.berkeley.edu"; DESC="Split tunnel (only campus/Berkeley traffic goes through the VPN)" ;;
...
echo "    switch : $CMD [split | full | restricted]   (run '$CMD help' for help)"
